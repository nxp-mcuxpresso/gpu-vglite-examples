.. zephyr:code-sample:: toolkit
   :name: VGLite toolkit sample
   :relevant-api: display_interface

   Demonstrates VGLite GPU–accelerated rendering of SVG (Scalable Vector Graphics), 
   highlighting the functionality and performance of the SVG graphics toolkit.

Overview
********

This example shows how to use the VGLite component for hardware-accelerated 2D
graphics rendering. This application draws graphics as per SVG (Scalable Vector
Graphics) on the screen, demonstrating VGLite's SVG rendering capabilities and
toolkit functionality.

The toolkit demo showcases VGLite's ability to parse and render SVG-based vector
graphics, providing a comprehensive demonstration of the VGLite toolkit features
including path rendering, transformations, and complex vector graphics composition.
...
.. code-block:: bash
   ...
   west build -p always -b mimxrt1170_evk/mimxrt1176/cm7 modules/gpu-vglite-examples/toolkit/ --shield rk055hdmipi4m -DBOARD_REVISION=A -DCONFIG_DEBUG=y -DCONFIG_DEBUG_OPTIMIZATIONS=n -DCMAKE_BUILD_TYPE=Debug -DVGLITE_OS=zephyr -DVGLITE_PLATFORM=rt1170
...
.. code-block:: bash
   ...
   west build -p always -b mimxrt595_evk/mimxrt595s/cm33 modules/gpu-vglite-examples/toolkit/ --shield rk055hdmipi4ma0 -- -DDTC_OVERLAY_FILE="$(west topdir)/modules/gpu-vglite-examples/soc/mimxrt595/mimxrt595_evk.overlay" -DCONFIG_DEBUG=y -DCONFIG_DEBUG_OPTIMIZATIONS=n -DCMAKE_BUILD_TYPE=Debug -DVGLITE_OS=zephyr -DVGLITE_PLATFORM=rt500
...
.. code-block:: bash
   ...
   west build -p always -b mimxrt1170_evk/mimxrt1176/cm7 modules/gpu-vglite-examples/toolkit/ --shield rk055hdmipi4m -DBOARD_REVISION=A -DVGLITE_OS=zephyr -DVGLITE_PLATFORM=rt1170

For :zephyr:board:`mimxrt700_evk`:

.. code-block:: bash

   west build -p always -b mimxrt700_evk/mimxrt798s/cm33_cpu0 modules/gpu-vglite-examples/toolkit/ --shield rk055hdmipi4ma0 -DVGLITE_OS=zephyr -DDTC_OVERLAY_FILE=$(west topdir)/modules/gpu-vglite-examples/soc/mimxrt700/mimxrt700_evk.overlay -DVGLITE_PLATFORM=rt700 -DEXTRA_CPPFLAGS="-DCPU_MIMXRT798SGFOA_cm33_core0"

For :zephyr:board:`mimxrt595_evk`:

.. code-block:: bash

   west build -p always -b mimxrt595_evk/mimxrt595s/cm33 modules/gpu-vglite-examples/toolkit/ --shield rk055hdmipi4ma0 -- -DDTC_OVERLAY_FILE="$(west topdir)/modules/gpu-vglite-examples/soc/mimxrt595/mimxrt595_evk.overlay" -DVGLITE_OS=zephyr -DVGLITE_PLATFORM=rt500

Running the Demo
================

When the example runs, you can see SVG-based vector graphics rendered on the screen.
The toolkit demo displays various graphical elements demonstrating VGLite's SVG
rendering capabilities, including complex paths, shapes, and transformations.
...
.. _SVG Specification: https://www.w3.org/TR/SVG2/
