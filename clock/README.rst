.. zephyr:code-sample:: clock
   :name: VGLite clock sample
   :relevant-api: display_interface

   Demonstrate VGLite GPU rendering with an animated clock display.

Overview
********

This example shows how to use the VGLite component for hardware-accelerated 2D
graphics rendering. This application draws an animated clock on the screen,
demonstrating real-time graphics updates and VGLite GPU rendering capabilities.

The clock demo showcases VGLite's ability to render dynamic graphics with smooth
animations, including clock hands movement.
...
.. code-block:: bash
   ...
   west build -p always -b mimxrt1170_evk/mimxrt1176/cm7 modules/gpu-vglite-examples/clock/ --shield rk055hdmipi4m -DBOARD_REVISION=A -DCONFIG_DEBUG=y -DCONFIG_DEBUG_OPTIMIZATIONS=n -DCMAKE_BUILD_TYPE=Debug -DVGLITE_OS=zephyr -DVGLITE_PLATFORM=rt1170
...
.. code-block:: bash
   ...
   west build -p always -b mimxrt595_evk/mimxrt595s/cm33 modules/gpu-vglite-examples/clock/ --shield rk055hdmipi4ma0 -- -DDTC_OVERLAY_FILE="$(west topdir)/modules/gpu-vglite-examples/soc/mimxrt595/mimxrt595_evk.overlay" -DCONFIG_DEBUG=y -DCONFIG_DEBUG_OPTIMIZATIONS=n -DCMAKE_BUILD_TYPE=Debug -DVGLITE_OS=zephyr -DVGLITE_PLATFORM=rt500
...
.. code-block:: bash
   ...
   west build -p always -b mimxrt1170_evk/mimxrt1176/cm7 modules/gpu-vglite-examples/clock/ --shield rk055hdmipi4m -DBOARD_REVISION=A -DVGLITE_OS=zephyr -DVGLITE_PLATFORM=rt1170

For :zephyr:board:`mimxrt700_evk`:

.. code-block:: bash

   west build -p always -b mimxrt700_evk/mimxrt798s/cm33_cpu0 modules/gpu-vglite-examples/clock/ --shield rk055hdmipi4ma0 -DVGLITE_OS=zephyr -DDTC_OVERLAY_FILE=$(west topdir)/modules/gpu-vglite-examples/soc/mimxrt700/mimxrt700_evk.overlay -DVGLITE_PLATFORM=rt700 -DEXTRA_CPPFLAGS="-DCPU_MIMXRT798SGFOA_cm33_core0"

For :zephyr:board:`mimxrt595_evk`:

.. code-block:: bash

   west build -p always -b mimxrt595_evk/mimxrt595s/cm33 modules/gpu-vglite-examples/clock/ --shield rk055hdmipi4ma0 -- -DDTC_OVERLAY_FILE="$(west topdir)/modules/gpu-vglite-examples/soc/mimxrt595/mimxrt595_evk.overlay" -DVGLITE_OS=zephyr -DVGLITE_PLATFORM=rt500

Running the Demo
================

When the example runs, you can see an animated clock displayed on the screen.
The clock hands move smoothly, demonstrating the VGLite GPU's capability for
